function [pos_err,speed_err,pos_cov,speed_cov]=freefall(Q,R)
...
pos_err = mean(abs(kalman_err_x));
speed_err = mean(abs(kalman_err_v));
pos_cov=err_P(size(err_P,1),1);
speed_cov=err_P(size(err_P,1),2);
end
% 
